  ...
  double acceptanceRatioBreakValue)
{
  ...
  double requiredLeafFARate = 1e-10;
  double tempLeafFARate;


  cout << endl << "===== Collecting Samples =====" << endl;
  cout << "<BEGIN" << endl;
...

  {
    ...
    int minutes = (int(seconds) / 60) % 60;
    int seconds_left = int(seconds) % 60;
    cout << "Writing data took " << days << " days " << hours << " hours " << minutes << " minutes " << seconds_left << " seconds." << endl;
  }
  cout << "Only collected data no cascade has been trained" << endl;
  return false;
}
