...
inline float CvCascadeBoost_CalculateLoss(float hitRate, float falseAlarm, int numTrees, double maxFalseAlarmParam, double costLimit)
{
  if (hitRate >= 0.9995f) {
    hitRate = 0.9995f;
  }
  ...
  // costLimit functions as soft limit to numTrees if maxFalseAlaramParam < 0.5
  // if maxFalseAlaramParam >= 0.5 it acts as a lower bound to acc_score instead
  if (maxFalseAlarmParam < 0.5 && timeCost > costLimit) {
    return (float)INT_MAX;
  }
  else if (maxFalseAlarmParam >= 0.5 && accScore < costLimit) {
    return (float)INT_MAX;
  }
...
float CvCascadeBoost::set_best_threshold()
{
  ...
  float minLoss = (float)INT_MAX;
  float bestThreshold = 0.5;
  float optHitrate = 0;
  float optFalseAlarm = 1;

  for (int _hitRateTry = 0; _hitRateTry < 100; _hitRateTry++) {
    ...
    if (loss < minLoss) {
      ...
      optHitrate = hitRate;
      optFalseAlarm = falseAlarm;
    }
  }

  cout << "|"; cout.width(4); cout << right << weak->total;
  cout << "|"; cout.width(9); cout << right << optHitrate;
  cout << "|"; cout.width(9); cout << right << optFalseAlarm;
  cout << "|"; cout.width(15); cout << right << minLoss;
  cout << "|" << endl;
  cout << "+----+---------+---------+---------------+" << endl;
